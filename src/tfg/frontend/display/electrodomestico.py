    ...
    def _actualizar_tiempo_uso(self, estado_toggle):
        if estado_toggle != self._esta_activado():
            if estado_toggle:
                ...
                st.session_state[self.session_key]["inicio_uso"] = datetime.datetime.now()
            else:
                # Detener uso y acumular tiempo
                if st.session_state[self.session_key]["activo"] and st.session_state[self.session_key]["inicio_uso"]:
                    tiempo_transcurrido = datetime.datetime.now() - st.session_state[self.session_key]["inicio_uso"]
                    st.session_state[self.session_key]["tiempo_acumulado"] += tiempo_transcurrido
