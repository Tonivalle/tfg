...
class PanelSolar(BaseModel):
    ...
    def calcular_generacion(self, serie_irradiacion: list[float]) -> float:
        """
        Calcula la energía generada total (W) mediante una serie de irradiacion por hora.
        """
        return sum(
            self.calcular_generacion_estimada_por_hora(irradiacion)
            for irradiacion in serie_irradiacion
        )

    def calcular_generacion_estimada_por_hora(self, irradiacion: float) -> float:
        """
